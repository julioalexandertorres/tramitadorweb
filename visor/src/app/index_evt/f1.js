...
try{
  ...
  function buildTableSelect(datostablas, select, filtro) {
   ...
   for (i = 0; i < datostablas.length; i++) {  
    ...
    return [
        '<a href="javascript:void(0)" class="verInfo" title="Ver detalle del registro">',
          '<i class="fa fa-info-circle fa-lg" style="color: #F8971C"></i>',
        '</a>'
      ].join("");
    },
    events: {
      "click .verInfo": function (e, value, row, index) {
        if(filtro == 'historialRegistro'){
          activarLoading();
          if(row[0] == 'Estado de embarazo'){
            datosHistorialeemb(row[1]);
          }
          else if(row[0] == 'Permiso remunerado' || row[0] == 'Permiso no remunerado'){
            datosHistorialpr(row[1]);
          }
          else if(row[0] == 'Retiro de personal'){
            datosHistorialrp(row[1]);
          }
          else if(row[0] == 'Reintegro de personal'){
            datosHistorialrip(row[1]);
          }
          else if(row[0] == 'Vacaciones'){
            datosHistorialv(row[1]);
          }
        }
        else{
          if(row[0] == 'Estado de embarazo'){
            datosColpendiente(row[1]);
          }
          else if(row[0] == 'Permiso remunerado' || row[0] == 'Permiso no remunerado'){
            datosColpendientepr(row[1]);
          }
          else if(row[0] == 'Retiro de personal'){
            datosColpendienterp(row[1]);
          }
          else if(row[0] == 'Reintegro de personal'){
            datosColpendienterip(row[1]);
          }
          else if(row[0] == 'Vacaciones'){
            datosColpendientev(row[1]);
          }
          /*if(filtro == 'pendienteAprobacionembarazo'){
          datosColpendiente(row[1]);
          }*/
          else if(filtro == 'historialRegistro'){
            datotablaHistorialderegistro(row);
          }         
         }
       }
    },
...
function exportexcel(){
  ...
  function enviarDatospendientesaprobacion( datos ){
    if (datos.length > 0){
        ...
        var datostablas = [['Tipo de absentismo'], ['Id Solicitud'], ['Nombre del colaborador'], ['Cédula'], ['Fecha de registro']];
        var datosTable = [];
  ...
  function enviarDatoshistorialregistro( datos ){
    if (datos.length > 0){
        ...
        $('body').removeClass('modal-open');
        $('.modal-backdrop').remove();
        var datostablas = [['Tipo de absentismo'], ['Id Solicitud'], ['Nombre del colaborador'], ['Cédula'], ['Fecha de registro'], ['Estado']];
        var datosTable = [];
        for (i=0; i < datos.length; i++){
          if(datos[i][16] == 'Estado de embarazo'){
